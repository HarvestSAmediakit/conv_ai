...
export default function AudioInterruptReader() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F6] text-zinc-900 font-sans selection:bg-[#00c896]/10 pb-24">
      {/* Top Header */}
      <header className="sticky top-0 bg-[#FAF9F6]/95 backdrop-blur-xl border-b border-zinc-200/60 z-50 p-4 shrink-0 shadow-xs flex justify-between items-center">
        <div className="flex items-center gap-3">
           ...
           <div>
              ...
              <h1 className="text-sm font-black text-zinc-900">The Future of AI Reader</h1>
           </div>
        </div>
        <div className="bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest border border-indigo-100 flex items-center gap-2 cursor-pointer shadow-sm">
      ...
      <main className="max-w-2xl mx-auto p-6 md:p-10 relative">
         <div className={`transition-all duration-700 ${fsmState === 'LISTENING' || fsmState === 'THINKING' || fsmState === 'ANSWERING' || fsmState === 'RESUMING' ? 'opacity-30 blur-[2px] scale-[0.98]' : 'opacity-100'}`}>
            ...
            <p className="text-lg leading-relaxed text-zinc-700 font-serif mb-6">
            ...
            {/* Example Ad Block */}
            <div className="my-8 rounded-3xl overflow-hidden border border-zinc-200 relative group group-hover:border-indigo-300 transition-colors">
            ...
            <p className="text-lg leading-relaxed text-zinc-700 font-serif">
         ...
         {(fsmState === 'LISTENING' || fsmState === 'THINKING' || fsmState === 'ANSWERING' || fsmState === 'RESUMING') && (
           <div className="fixed inset-0 top-16 z-40 bg-zinc-900/10 backdrop-blur-[1px] flex items-center justify-center p-4">
              <div className="bg-white rounded-[2rem] border border-zinc-200 shadow-2xl w-full max-w-sm p-6 transform transition-all animate-in zoom-in-95 duration-300">
                 
                 {/* User Query bubble */}
                 <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-2xl rounded-tr-sm mb-4">
                 ...
                 <div className="flex gap-4">
                   ...
                   <div className="flex-1">
                      ...
                      {fsmState === 'ANSWERING' && (
                        <p className="text-zinc-800 text-sm leading-relaxed animate-in fade-in slide-in-from-bottom-2">
                      ...
                      {fsmState === 'RESUMING' && (
                        <div className="space-y-4">
                          <p className="text-zinc-800 text-sm leading-relaxed text-opacity-50">
                             Yes! The current ad on page 12 mentions a 15% discount...
                          </p>
                          <div className="bg-zinc-50 border border-zinc-100 p-3 rounded-xl flex items-center justify-between">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-xl border-t border-zinc-200 p-4 safe-area-pb z-50 flex items-center justify-center">
         <div className="w-full max-w-sm flex items-center justify-between gap-4">
            ...
            <button className="p-3 text-zinc-400 hover:text-zinc-800 transition-colors relative group">
